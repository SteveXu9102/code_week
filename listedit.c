...
unit *findByName(char *name, unit *head) {     // 链表查找函数，参数为需要查找的条目名称字符串和链表首部指针
    unit *mid = head;  // 避免直接修改头指针
    if (mid->next != NULL && strcmp(mid->col.name, name) == 0) {  //对首部节点的查找比较
        return mid;    // 查找到后返回地址
    }
    while (mid->next != NULL) {    // 对后续节点的查找比较
        if (strcmp(mid->next->col.name, name) == 0) {  // 比较名称
...
int del(char *name, unit *L) {  // 删除节点，参数为待删除的节点名字符串和链表首部指针，返回值表示操作是否成功
    unit *f = L, *m; // f: 避免直接修改L指向，m: 存储地址的中间量
    m = findByName(name, f);  // 搜索待删除节点
    if (m == L) {   // 待删除节点恰为首部
        L = m->next;  // 修改首部指针
        free(m);  // 释放原首部的内存
        return 0;
        ...
        while (f->next != NULL) {
            if (f->next == m) break;  // 使f指向待删除节点的前一节点
            f = f->next;
        }
        free(m);  // 释放节点内存
        f->next = n;  // 链接该节点前后的节点
